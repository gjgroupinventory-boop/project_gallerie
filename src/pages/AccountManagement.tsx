  ...
  <div className="space-y-6 pt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
    {activeModalTab === 'permissions' && (
      <div className="space-y-6">
        ...
        <div className="pt-4 border-t border-neutral-100">
          <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-4">Gallery Operations Tab Control</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { key: 'canViewOpsBranches', label: 'Branches (Ops)' },
              { key: 'canViewOpsExhibitions', label: 'Exhibitions (Ops)' },
              { key: 'canViewOpsAuctions', label: 'Auctions (Ops)' },
              { key: 'canViewOpsSales', label: 'Sales (Ops)' },
              { key: 'canViewOpsReservations', label: 'Reservations (Ops)' },
              { key: 'canViewOpsInventory', label: 'Inventory (Ops)' },
              { key: 'canViewOpsReturnToArtist', label: 'Return to Artist (Ops)' },
              { key: 'canViewOpsForFraming', label: 'For Framing (Ops)' },
              { key: 'canViewOpsMonitoring', label: 'Monitoring (Ops)' },
            ].map(({ key, label }) => (
              <label key={key} className="flex items-center space-x-3 cursor-pointer group">
                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all duration-200 ${formData.permissions[key as keyof UserPermissions]
                  ? 'bg-neutral-900 border-neutral-900 text-white shadow-sm'
...
const AccountManagement: React.FC<AccountManagementProps> = ({ 
  ...
  const filteredAccounts = accounts.filter(acc => {
    ...
    if (activeTab === 'staff') {
      if (acc.role === UserRole.EXCLUSIVE) return false;
    } else {
      if (acc.role !== UserRole.EXCLUSIVE) return false;
    }
    ...
    if (searchQuery.trim()) {
      ...
      const matchBranch = (acc.branch || '').toLowerCase().includes(q);
      if (!matchName && !matchEmail && !matchBranch) return false;
    }

    // Filter by Role Type
    if (roleFilter !== 'All') {
      if (acc.role !== roleFilter) return false;
    }
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white text-neutral-900 rounded-md p-6 border border-neutral-200 shadow-lg space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              {/* Gallery Operations Tab Control */}
              <div className="bg-neutral-50/40 rounded-lg p-5 border border-neutral-200/60 space-y-4 shadow-sm hover:shadow-md transition-shadow">
                <h4 className="text-xs font-bold text-neutral-900 uppercase tracking-wider border-b border-neutral-200/80 pb-2.5 flex items-center justify-between">
                  <span>Gallery Operations Tab Control</span>
                  <span className="text-[10px] text-neutral-400 font-normal normal-case">Gallery Operations sub-tab visibilities</span>
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                  {[
                    { key: 'canViewOpsBranches', label: 'Branches (Ops)' },
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-md border border-neutral-200 shadow-sm">
        <div className="relative flex-1 max-w-md w-full">
          <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 text-neutral-400 w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-neutral-200 rounded-md text-xs font-semibold text-slate-700 placeholder-neutral-400 focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900 outline-none transition-all"
          />
        </div>
        ...
        {activeTab === 'staff' && (
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <select
              ...
              className="px-3 py-2 bg-white border border-neutral-200 rounded-md text-[11px] font-bold text-neutral-700 focus:border-neutral-900 outline-none transition-all cursor-pointer shadow-sm"
            >
